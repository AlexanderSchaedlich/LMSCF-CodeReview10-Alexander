...
<body>
	...
	<div class="container">
		<?php 
			$sql = "select 
			media.id, media.title, media.image, media.short_description, 
			...
			if($result->num_rows > 0){
				...
				while($row = $result->fetch_assoc()) {
					if($counter == 0){
						echo "
							<header>
								<h1 class='text-center p-5'>Media from " . $row['name'] . "</h1>
							</header>

							<main>
								<a class='btn btn-secondary' href='manage/insert.php' 
								style='margin-left: 15px;''>Insert</a>
								<br><br>
						";
						$counter++;
					}
